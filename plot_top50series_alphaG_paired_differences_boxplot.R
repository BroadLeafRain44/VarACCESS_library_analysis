#!/usr/bin/env Rscript
#
# Paired AlphaGenome DNase score differences: CAD caQTL + top50 0 vs 10/20 mut.
#
...
cad_selected_path <- file.path(out_dir, "top50series_alphaG_paired_differences_cad_most_negative50.tsv")
plot_path <- file.path(out_dir, "top50series_alphaG_paired_differences_boxplot.png")
stats_path <- file.path(out_dir, "top50series_alphaG_paired_differences_boxplot_stats.tsv")
...
cad <- data.frame(
  comparison = "CAD",
  difference = cad_dnase$raw_score_sum[seq_len(cad_n)],
...
paired <- rbind(
  data.frame(comparison = cad$comparison, difference = cad$difference, stringsAsFactors = FALSE),
  data.frame(comparison = "0 vs 10", difference = top50$raw_score_sum[top50$pos_in_series == 1L],
             stringsAsFactors = FALSE),
  data.frame(comparison = "0 vs 20", difference = top50$raw_score_sum[top50$pos_in_series == 2L],
             stringsAsFactors = FALSE)
)
paired <- paired[!is.na(paired$difference), , drop = FALSE]
paired$comparison <- factor(paired$comparison, levels = c("CAD", "0 vs 10", "0 vs 20"))

stats_out <- do.call(rbind, lapply(levels(paired$comparison), function(cmp) {
  vals <- paired$difference[paired$comparison == cmp]
  data.frame(
    comparison = cmp, n = length(vals), mean_difference = mean(vals),
...
p <- ggplot(paired, aes(x = comparison, y = difference)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.12, alpha = 0.6, size = 1.5) +
  labs(x = NULL, y = "AlphaGenome DNase score difference") +
  theme_classic(base_size = 12)

ggsave(plot_path, p, width = 7, height = 5, dpi = 150)
cat("Saved:", plot_path, "\n")
